package yanzichao.servlet;

import common.dao.IIncomeDAO;
import yanzichao.bean.Income;
import yanzichao.dao.IncomeDAO;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.List;

@WebServlet(name = "GetAllIncome",urlPatterns = "/GetAllIncome")
public class GetAllIncome extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        IIncomeDAO iIncomeDAO = new IncomeDAO();
        List<Income> incomes = iIncomeDAO.getAllincome();
        request.setAttribute("AllIncome", incomes);
        request.getRequestDispatcher("ShowIncome.jsp").forward(request, response);
    }

    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        doPost(request,response);
    }
}
